...
class ShadeCalc {
  ...
  calculatePathQuotaByEnding({endingTokenAmount: endingTokenAmount, endingTokenId: endingTokenId, path: path,}) {
    const {getTokenDecimals: getTokenDecimals} = (0, shade_api_utils_1.useTokens)(), {
      ...
      const token0Decimals = getTokenDecimals(poolPairInfo.token0Id),
        ...
        token0AmountInDenom = poolPairInfo.token0Amount.multipliedBy(Math.pow(10, token0Decimals)),
        token1AmountInDenom = poolPairInfo.token1Amount.multipliedBy(Math.pow(10, token1Decimals)),
        inputTokenDecimals = getTokenDecimals(inputTokenId), inputAmount = quoteInputAmount.toString(),
        l = (0, utils_1.convertCoinFromUDenomV2)(inputAmount, inputTokenDecimals);
      let u;
  ...
  calculatePathOutcome({startingTokenAmount: startingTokenAmount, startingTokenId: startingTokenId, path: path,}) {
    const g = (0, shade_api_utils_1.useTokens)(), {getTokenDecimals: getTokenDecimals} = g,
      pathReduceResult = path.reduce((pathSegment, poolId) => {
        ...
        const token0Decimals = getTokenDecimals(poolPairInfo.token0Id),
          ...
          token0AmountInDenom = poolPairInfo.token0Amount.multipliedBy(Math.pow(10, token0Decimals)),
          token1AmountInDenom = poolPairInfo.token1Amount.multipliedBy(Math.pow(10, token1Decimals)),
          outputTokenDecimals = getTokenDecimals(outputTokenId), outputAmountString = quoteOutputAmount.toString(),
          inputToken0Amount = (0, utils_1.convertCoinFromUDenomV2)(outputAmountString, outputTokenDecimals);
        let otherTokenId;
