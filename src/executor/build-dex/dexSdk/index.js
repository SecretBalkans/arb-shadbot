...
function calculateTokenSwap(dex, swapTokenSent, swapTokenReceived, route, amount) {
    ...
    switch (dex) {
        case "osmosis":
            ...
            const calc = new osmosis_calc_1.default(osmosisRoute.raws.map(({ pool: poolRaw }) => {
            ...
            const { denom: tokenInDenomOsmo, decimals: tokenInOsmoDecimals } = osmosisRoute.t0.symbol === swapTokenSent ? osmosisRoute.t0 : osmosisRoute.t1;
            const { denom: tokenOutDenomOsmo, decimals: tokenOutOsmoDecimals } = osmosisRoute.t0.symbol === swapTokenReceived ? osmosisRoute.t0 : osmosisRoute.t1;
