...
const logger = new Logger('ChainInfos');
const Bech32Address = {
...
export function getChainInfoByApproxName(chainName: string): SimplifiedChainInfo | null {
  const chainInfo = _.find(ChainInfos, d => d.chainName.includes(chainName));
  if (!chainInfo) {
    logger.debugOnce(`No chain info for name=${chainName}`);
  }
  return (chainInfo as SimplifiedChainInfo) || null;
}
